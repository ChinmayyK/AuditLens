...
import type { ReviewRequest, ReviewResult, ScoreResult, ReviewComment, ReEvaluateRequest } from '../types/review'
...
export async function submitPRReviewPublic(request: ReviewRequest): Promise<ReviewResult> {
  ...
  const res = await axios.post<ReviewResult>(
    `${getApiBaseUrl()}/review/pr-public`,
    request,
    {
...
export async function reEvaluateReview(request: ReEvaluateRequest): Promise<ReviewResult> {

...
export default function ScanDetailPage() {
  ...
  const sevMap: Record<string, BadgeVariant> = { critical: 'critical', high: 'high', medium: 'medium', low: 'low', info: 'info' }
  const statusMap: Partial<Record<NonNullable<typeof scan>['status'], BadgeVariant>> = {
    complete: 'success',
    failed: 'critical',
    running: 'blue',
  }
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <Badge variant={(scan?.status && statusMap[scan.status]) || 'default'}>
                    {scan?.status}
                  </Badge>

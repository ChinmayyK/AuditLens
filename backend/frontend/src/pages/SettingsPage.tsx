...
import { User, Key, Calendar, Plus, Trash2, Copy, X } from 'lucide-react'
...
function ProfileTab() {
  const { data: profile, isLoading } = useQuery<Profile>({ queryKey: ['profile'], queryFn: () => api.get<Profile>('/settings/profile').then(r => r.data) })
...
function APIKeysTab() {
  ...
  const { data: keys = [], isLoading } = useQuery<ApiKey[]>({ queryKey: ['apikeys'], queryFn: () => api.get<ApiKey[]>('/settings/api-keys').then(r => r.data) })
...
function SchedulesTab() {
  const qc = useQueryClient()
  const { data: schedules = [], isLoading } = useQuery<Schedule[]>({ queryKey: ['schedules'], queryFn: () => api.get<Schedule[]>('/settings/schedules').then(r => r.data) })

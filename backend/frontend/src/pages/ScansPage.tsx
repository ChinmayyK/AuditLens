...
function ScanProgress({ scan }: { scan: RecentScan }) {
  const progress = Math.max(0, Math.min(100, scan.progress_pct ?? (scan.status === 'complete' ? 100 : 0)))
  const isActive = scan.status === 'queued' || scan.status === 'running'
  const label = scan.status === 'queued'
    ? 'Waiting for worker'
    : scan.current_phase || (scan.status === 'complete' ? 'Complete' : scan.error_message || 'Not started')
  ...
  return (
    <div className="min-w-[180px]">
      ...
      <div className="h-1.5 rounded-full bg-slate-800 overflow-hidden border border-slate-700/70">
        <div
...
export default function ScansPage() {
  ...
  const { data: scans = [], isLoading, refetch } = useQuery({
    queryKey: ['scans'],
    queryFn: getScans,
    refetchInterval: (query) => (query.state.data && hasRunning(query.state.data)) ? 2000 : false,
  })
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(err, 'Failed to start scan'))
    } finally { setSubmitting(false) }
  }

  const riskVariant = (grade?: string): BadgeVariant =>
    grade && ['A', 'B'].includes(grade) ? 'success' : grade === 'C' ? 'warning' : 'critical'
              ...
              <tbody>
                {(scans as RecentScan[]).map(scan => (
                  <tr key={scan.id} onClick={() => navigate(`/scans/${scan.id}`)} className="border-b border-slate-700/60 hover:bg-slate-800/60 cursor-pointer transition-colors">
                    ...
                    <td className="px-5 py-3"><ScanProgress scan={scan} /></td>
                    <td className="px-5 py-3">{scan.risk_grade ? <Badge variant={riskVariant(scan.risk_grade)}>{scan.risk_grade}</Badge> : '—'}</td>
                ...
                {!(scans as RecentScan[]).length && <tr><td colSpan={7} className="px-5 py-12 text-center text-slate-500">No scans yet. Click "New Scan" to start.</td></tr>}
              </tbody>
            </table>
          )}
        </div>
